  ...
  return (
    <>
      <header className="fixed inset-x-0 top-0 z-50 transition-all duration-500 pointer-events-none">
        <div
          className={cn(
            "pointer-events-auto mx-auto flex max-w-7xl items-center gap-3 px-4 transition-all duration-500 sm:px-6",
            scrolled
      ...
      {/* Mobile overlay attached via portal to document body */}
      {mounted &&
        createPortal(
          <div
            id="mobile-nav-overlay"
            className={cn(
              "fixed inset-0 z-40 bg-background/98 backdrop-blur-2xl transition-all duration-300 xl:hidden overflow-y-auto pt-24 pb-12 h-[100dvh] w-screen",
              open
          ...
          >
            <nav
              className="flex min-h-[calc(100dvh-9rem)] flex-col justify-center gap-1 px-8 max-w-md mx-auto"
              aria-label="Mobile"
            >
              {navLinks.map((l, i) => (
                <Link
